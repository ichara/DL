...
class ReplayMemory:
    ...
    def add(self, s1, action, s2, done, reward, energy, distance):
        with self.lock:
            self.s1[self.pos, :, :, :] = s1
            ...
            self.distance[self.pos] = distance
    ...
    def sample(self, sample_size):
        ...
        with self.lock:
            return self.s1[i], self.a[i], self.s2[i], self.done[i], self.r[i], self.energy[i], self.distance[i]
